        ...
        <div className="fixed bottom-20 right-5 w-80 bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col z-50 overflow-hidden"
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-gray-50">
            ...
            {messages.length === 0 && (
              <div className="space-y-3">
                <div className="bg-white rounded-2xl rounded-tl-sm px-3 py-2.5 shadow-sm border border-gray-100 text-sm text-gray-700">
                  ¡Hola! Soy el asistente de BotWA 👋 Podés preguntarme cualquier cosa sobre la plataforma.
                </div>
                <div className="space-y-1.5">
                  {HELP_SUGGESTED_QUESTIONS.map(q => (
                    <button
                      key={q}
                      onClick={() => send(q)}
                      className="w-full text-left text-xs px-3 py-2 bg-white border border-indigo-100 text-indigo-700 rounded-xl hover:bg-indigo-50 transition-colors shadow-sm"
                    >
                      {q}
                    </button>
                  ))}
          ...
          <div className="px-3 py-2.5 border-t border-gray-100 bg-white shrink-0 flex gap-2">
            <input
              ...
              className="flex-1 text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
            />
            <button
              onClick={() => send(input)}
